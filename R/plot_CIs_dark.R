...
plot_CIs_dark<-function (df, title, plot.contrasts, plot.contrasts.2, ypos, type, offset) {
  ...
  p <- ggplot(df, aes(x=genotype)) +
    theme_solarized(light=FALSE) +#x-layer
  ...
  if(type == "dauer") {
    p1 <- p + 
      geom_dotplot(aes(y=pct),binwidth=.015, binaxis="y", position="dodge", 
                   stackdir="center", size =.3, fill ="blue", colour = "blue") +
